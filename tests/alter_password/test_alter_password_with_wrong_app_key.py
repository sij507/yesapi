...
class TestAlterPasswordWithWrongAppKey:
    ...
    def test_login_with_wrong_app_key(self):
        with allure.step('Send alter password request with wrong app key'):
            alter_password_with_wrong_app_key = SendRequest(AlterPassword.get_test_case("test_alter_password_with_wrong_app_key"))
        ...
        with allure.step('Checking msg'):
            allure.attach("Msg " + constants.ERR_MSG_ALTER_PSW_WRONG_APP_KEY, "Expected:")
            allure.attach("Msg: " + alter_password_with_wrong_app_key.get_msg(), "Actual:")
            assert alter_password_with_wrong_app_key.get_msg() == constants.ERR_MSG_ALTER_PSW_WRONG_APP_KEY
