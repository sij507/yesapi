...
class TestAlterPasswordWithLongPassword:
    ...
    def set_up(self):
        with allure.step('Set up alter password params'):
            TestAlterPasswordWithLongPassword.params = Params.setup_alter_password_params(
                constants.REGULAR_USERNAME, constants.WRONG_MD5_PASSWORD, constants.NEW_PASSWORD,
                constants.APP_KEY)
    ...
    def test_alter_password_with_wrong_password(self):
        with allure.step('Send an alter password request with wrong password'):
            alter_password_wrong_username = SendRequest(constants.GET, constants.BASE_URL,
                                                        TestAlterPasswordWithLongPassword.params)
