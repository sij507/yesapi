...
class TestLogoutWithLongUuid:
    ...
    def test_log_out_with_admin_success(self):
        with allure.step('Set up admin logout params'):
            params = Params.setup_logout_params(constants.LONG_UUID, TestLogoutWithLongUuid.admin_token,
                                                constants.APP_KEY)

        with allure.step("Send an admin logout request"):
            manager_logout_with_long_uuid = SendRequest(constants.GET, constants.BASE_URL, params)
